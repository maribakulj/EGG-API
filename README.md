...
A lightweight server-rendered admin interface is available at:

- `GET /admin/login` (sign in with admin API key)
- `GET /admin/ui` (dashboard)

After sign in, the UI provides pages for:
- **Dashboard**: service/backend health summary, paths, key/event counts
- **Configuration**: safe editable core settings and profile limits
- **Mapping**: read-only inspection of field mapping and exposure allowlists
- **API keys**: list/create/activate/suspend/revoke keys (new secret shown once)
- **Recent activity**: persisted request log table

Authentication model:
- Sign-in uses existing admin API key validation
- A short-lived UI session cookie is issued after successful sign-in
- UI routes redirect to `/admin/login` when unauthenticated
